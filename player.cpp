    ...
    ui(new Ui::player)
{
    ...
    qDebug()<<"Transition time is now set to: "<<mediaObject->transitionTime();

     /* LP1*/

     audioOutputLp1 = new Phonon::AudioOutput(Phonon::MusicCategory, this);
     mediaObjectLp1 = new Phonon::MediaObject(this);
     //metaInformationResolverLp1 = new Phonon::MediaObject(this);

     dataoutLp1 = new Phonon::AudioDataOutput(this);
     Phonon::createPath(mediaObjectLp1, audioOutputLp1);
     Phonon::createPath(mediaObjectLp1, dataoutLp1);
     connect( dataoutLp1, SIGNAL(dataReady(const QMap<Phonon::AudioDataOutput::Channel, QVector<qint16> >)),
             this, SLOT(dataReceivedLp1(const QMap<Phonon::AudioDataOutput::Channel, QVector<qint16> >)));
    ui->volumeSlider_LP1->setAudioOutput(audioOutputLp1);
    ui->seekSlider_LP1->setMediaObject(mediaObjectLp1);

     /* LP2*/

     audioOutputLp2 = new Phonon::AudioOutput(Phonon::MusicCategory, this);
     mediaObjectLp2 = new Phonon::MediaObject(this);
     //metaInformationResolverLp2 = new Phonon::MediaObject(this);

     dataoutLp2 = new Phonon::AudioDataOutput(this);
     Phonon::createPath(mediaObjectLp2, audioOutputLp2);
     Phonon::createPath(mediaObjectLp2, dataoutLp2);
     connect( dataoutLp2, SIGNAL(dataReady(const QMap<Phonon::AudioDataOutput::Channel, QVector<qint16> >)),
             this, SLOT(dataReceivedLp2(const QMap<Phonon::AudioDataOutput::Channel, QVector<qint16> >)));
     ui->volumeSlider_LP2->setAudioOutput(audioOutputLp2);
     ui->seekSlider_LP2->setMediaObject(mediaObjectLp2);
...
*/
   ...
   mediaObject->setCurrentSource(QUrl::fromLocalFile("/home/fred/cpp/config/xfb.ogg"));
   mediaObject->play();
...
void player::playNextSong(){
    ...
    if(PlayMode=="Playing_Segue"){
        ...
        if(ui->playlist->count()>0){
            ...
            mediaObject->setCurrentSource(QUrl::fromLocalFile(itemDaPlaylist));
            mediaObject->play();
            ...
            if((lastPlayedSong!=itemDaPlaylist)||(autoMode==0)){
                ...
                if(hlistcout > 999){
                    //qDebug()<<"HistoryList is beeing cleaned beacuse it's over 1000 records now...";
                    delete ui->historyList->item(0);
                }

                mediaObject->play();

                qDebug()<< "mediaObject->state() is: " << mediaObject->state();
...
void player::onPositionChanged(qint64 position)
{
     ...
     //if(position%2==0)
        //qDebug()<<"mediaObject on position changed to "<<position;
...
{
    qDebug()<<"mediaObject durationChanged changed to "<<position;
    //ui->seekSlider->setMaximum(position);
...
void player::update_music_table(){
    ...
    jinglesmodel->setTable("jingles");
    jinglesmodel->select();
    jinglesmodel->sort(1, Qt::AscendingOrder);
    ui->jinglesView->setModel(jinglesmodel);
    ...
    pubmodel->setTable("pub");
    pubmodel->select();
    pubmodel->sort(1, Qt::AscendingOrder);
    ui->pubView->setModel(pubmodel);
    ...
    programsmodel->setTable("programs");
    programsmodel->select();
    programsmodel->sort(1, Qt::AscendingOrder);
    ui->programsView->setModel(programsmodel);
    ...
    qry->exec(sqlq);
    model_genre1->setQuery(*qry);
    model_genre1->sort(1, Qt::AscendingOrder);
    ui->cBoxGenre1->setModel(model_genre1);
    ui->cBoxGenre2->setModel(model_genre1);
...
void player::run_scheduler(){
    ...
    /*check if it's time to stop or start*/

    if(AutoStartStop == "true"){
        QTime time = QTime::currentTime();
        QString stime = time.toString();
        QStringList array_time = stime.split(":");
        QString hhmm = array_time[0]+":"+array_time[1];

        qDebug()<<"Current time: "<<time.toString();
        if(AutoStartTime == hhmm){
            ...
            qDebug()<<"Auto Starting to Play as set in options.";
        } else if(AutoStopTime == hhmm){
            mediaObject->stop();
            ui->btPlay->setStyleSheet("");
            ui->btPlay->setText(tr("Play"));
            PlayMode = "stopped";
          //  Xplaylist->removeMedia(0);
            qDebug()<<"Auto Stoping as set in options.";
        }
...
void player::on_bt_search_clicked()
{
    ...
    model->setQuery("select * from musics where artist like '%"+term+"%' or song like '%"+term+"%' ORDER BY artist ASC");
    ui->musicView->setModel(model);
...
void player::RectimerDone(){
    ...
    audioRecorder->setAudioInput(ui->audioDeviceBox->currentText());

    QAudioEncoderSettings settings;
    settings.setCodec(ui->audioCodecBox->currentText());
    settings.setSampleRate(ui->sampleRateBox->currentText().toInt());
    settings.setBitRate(ui->bitrateBox->currentText().toInt());
    settings.setChannelCount(ui->channelsBox->currentText().toInt());
    settings.setQuality(QMultimedia::EncodingQuality(ui->qualitySlider->value()));
    settings.setEncodingMode(QMultimedia::ConstantQualityEncoding);

    QString container = ui->containerBox->currentText();
...
void player::on_bt_rol_streaming_play_clicked()
{
    ...
    QString serverIpNPortNStream = ui->lineEdit_streamingClientAddr->text();



    QString radio1str = "mplayer -playlist "+serverIpNPortNStream;
...
void player::on_lp_1_bt_play_clicked()
{
    ...
    mediaObjectLp1->setCurrentSource(QUrl::fromLocalFile(ui->lp_1_txt_file->text()));
    mediaObjectLp1->play();

    qDebug()<< "mediaObjectLp1->state() is: " << mediaObjectLp1->state();
...
void player::on_lp_1_bt_play_2_clicked()
{
    ...
    mediaObjectLp2->setCurrentSource(QUrl::fromLocalFile(ui->lp_2_txt_file->text()));
    mediaObjectLp2->play();

    qDebug()<< "mediaObjectLp2->state() is: " << mediaObjectLp2->state();
...
{
    qDebug()<<"Starting all streaming services";
...
void player::on_bt_icecast_clicked()
{
    ...
    if(icecastrunning==false){
        ...
        qDebug()<<"Starting icecast with the config file: /etc/icecast2/icecast.xml";
        processo.startDetached("sh",QStringList()<<"-c"<<"xvfb-run -a icecast2 -c /etc/icecast2/icecast.xml");
        ...
        ui->bt_icecast->setStyleSheet("background-color:#C8EE72"); //green

        /*This timmer will check if icecast2 is still running*/
...
//
    QString ptest = "ss -tl4 '( sport = :9999 )' | grep LISTEN";
    QString nctest = "nc -z "+ui->lbl_ddns->text()+" 9999; echo $?";


    qDebug()<<"The ss command to check the port is: "<<ptest;
    qDebug()<<"The nc command to check the port is: "<<nctest;
    ...
    if(cmdout != ""){
        ...
        if(ncmdout != ""){
            ...
            QString lbl_streamURL = "<a href=\"http://"+ui->lbl_ddns->text()+":9999/stream.m3u\">http://"+ui->lbl_ddns->text()+":8888/stream.m3u</a>";
            ...
            QMessageBox::information(this,tr("Error"),tr("It was not possible to determine if port 9999 is open/forwarded :-( \n"
        ...
        QMessageBox::information(this,tr("Error"),tr("It was not possible to determine if port 9999 is LISTENING :-( \n"
...
void player::on_bt_takeOver_clicked()
{
    ...
    if(takeOver == false){
        ...
        QString ip = ui->lbl_ddns->text();
        QString stream = "http://"+ip+":9999/stream.m3u";
...
void player::MainsetVol100(){
    //mediaObject->setVolume(100);
}

void player::MainsetVol80(){
   // mediaObject->setVolume(80);
}
void player::MainsetVol60(){
  //  mediaObject->setVolume(60);
}
void player::MainsetVol40(){
//    mediaObject->setVolume(40);
}
void player::MainsetVol20(){
  //  mediaObject->setVolume(20);
}
void player::MainsetVol10(){
  //  mediaObject->setVolume(10);
}
void player::MainsetVol5(){
  //  mediaObject->setVolume(5);
}
...
void player::pingTakeOverClient(){
    ...
    if(pingArray[1]==" 1 received"){
        ...
        //stop mplayer and start mediaObject
...
void player::on_actionUpdate_Configuration_triggered()
{
    updateConfig();
}

void player::on_bt_purgeCache_clicked()
{
    qDebug()<<"Purging / Cleaning Cache...";


    QString rmthis = TakeOverPath+"/takeover.xml";
    QFile::remove(rmthis);

    rmthis = TakeOverPath+"/confirmtakeover.xml";
    QFile::remove(rmthis);


    rmthis = TakeOverPath+"/returntakeover.xml";
    QFile::remove(rmthis);


    qDebug()<<"TakeOverPath cleared.";


    rmthis = FTPPath+"/takeover.xml";
    QFile::remove(rmthis);

    rmthis = FTPPath+"/confirmtakeover.xml";
    QFile::remove(rmthis);


    rmthis = FTPPath+"/returntakeover.xml";
    QFile::remove(rmthis);
    qDebug()<<"FTPPath cleared.";
...
void player::dataReceived(const QMap<Phonon::AudioDataOutput::Channel, QVector<qint16> > map){
    ...
    int valorCanalDireito = map[Phonon::AudioDataOutput::RightChannel][0];
    //  qDebug () << "Valor do canal direito: " << abs(valorCanalDireito);

    int valorCanalEsquerdo = map[Phonon::AudioDataOutput::LeftChannel][0];
    // qDebug () << "Valor do canal esquerdo: " << abs(valorCanalEsquerdo);


    //Valor da coluna da direita | Right speaker values
    int valorPercentualDireito = abs(valorCanalDireito/90);
    if(valorPercentualDireito>0){




        if(valorPercentualDireito > 95){
             ui->VU_MAIN_R10->setPixmap(QPixmap(":/icons/led_red_on.png"));
        } else {
            ui->VU_MAIN_R10->setPixmap(QPixmap(":/icons/led_red_off.png"));
        }
        if(valorPercentualDireito > 90){
             ui->VU_MAIN_R9->setPixmap(QPixmap(":/icons/led_yellow_on.png"));
        } else {
            ui->VU_MAIN_R9->setPixmap(QPixmap(":/icons/led_yellow_off.png"));
        }
        if(valorPercentualDireito > 80){
             ui->VU_MAIN_R8->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_R8->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(valorPercentualDireito > 70){
             ui->VU_MAIN_R7->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_R7->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(valorPercentualDireito > 60){
             ui->VU_MAIN_R6->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_R6->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(valorPercentualDireito > 50){
             ui->VU_MAIN_R5->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_R5->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(valorPercentualDireito > 40){
             ui->VU_MAIN_R4->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_R4->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }
        if(valorPercentualDireito > 30){
             ui->VU_MAIN_R3->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_R3->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }
        if(valorPercentualDireito > 20){
             ui->VU_MAIN_R2->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_R2->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }
        if(valorPercentualDireito > 10){
             ui->VU_MAIN_R1->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_R1->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }



       // qDebug () << "Valor do canal direito: " << valorPercentualDireito;
    }




    //Valor da coluna da esquerda | Left speaker values
    int valorPercentualEsquerdo = abs(valorCanalEsquerdo/90);
    if(valorPercentualEsquerdo>0){



        if(valorPercentualEsquerdo > 95){
             ui->VU_MAIN_L10->setPixmap(QPixmap(":/icons/led_red_on.png"));
        } else {
            ui->VU_MAIN_L10->setPixmap(QPixmap(":/icons/led_red_off.png"));
        }
        if(valorPercentualEsquerdo > 90){
             ui->VU_MAIN_L9->setPixmap(QPixmap(":/icons/led_yellow_on.png"));
        } else {
            ui->VU_MAIN_L9->setPixmap(QPixmap(":/icons/led_yellow_off.png"));
        }
        if(valorPercentualEsquerdo > 80){
             ui->VU_MAIN_L8->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_L8->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(valorPercentualEsquerdo > 70){
             ui->VU_MAIN_L7->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_L7->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(valorPercentualEsquerdo > 60){
             ui->VU_MAIN_L6->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_L6->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(valorPercentualEsquerdo > 50){
             ui->VU_MAIN_L5->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_L5->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(valorPercentualEsquerdo > 40){
             ui->VU_MAIN_L4->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_L4->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }
        if(valorPercentualEsquerdo > 30){
             ui->VU_MAIN_L3->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_L3->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }
        if(valorPercentualEsquerdo > 20){
             ui->VU_MAIN_L2->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_L2->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }
        if(valorPercentualEsquerdo > 10){
             ui->VU_MAIN_L1->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_L1->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }


        //qDebug () << "Valor do canal Esquerdo: " << valorPercentualEsquerdo;
    }


    }
void player::dataReceivedLp1(const QMap<Phonon::AudioDataOutput::Channel, QVector<qint16> > map){
...
if(valorPercentualDireito>0){
    ...
    if(valorPercentualDireito > 95){
         ui->VU_LP1_R10->setPixmap(QPixmap(":/icons/led_red_on.png"));
    } else {
        ui->VU_LP1_R10->setPixmap(QPixmap(":/icons/led_red_off.png"));
    }
    if(valorPercentualDireito > 90){
         ui->VU_LP1_R9->setPixmap(QPixmap(":/icons/led_yellow_on.png"));
    } else {
        ui->VU_LP1_R9->setPixmap(QPixmap(":/icons/led_yellow_off.png"));
    }
    if(valorPercentualDireito > 80){
         ui->VU_LP1_R8->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP1_R8->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualDireito > 70){
         ui->VU_LP1_R7->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP1_R7->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualDireito > 60){
         ui->VU_LP1_R6->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP1_R6->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualDireito > 50){
         ui->VU_LP1_R5->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP1_R5->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualDireito > 40){
         ui->VU_LP1_R4->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP1_R4->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualDireito > 30){
         ui->VU_LP1_R3->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP1_R3->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualDireito > 20){
         ui->VU_LP1_R2->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP1_R2->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualDireito > 10){
         ui->VU_LP1_R1->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP1_R1->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
...
if(valorPercentualEsquerdo>0){
    ...
    if(valorPercentualEsquerdo > 95){
         ui->VU_LP1_L10->setPixmap(QPixmap(":/icons/led_red_on.png"));
    } else {
        ui->VU_LP1_L10->setPixmap(QPixmap(":/icons/led_red_off.png"));
    }
    if(valorPercentualEsquerdo > 90){
         ui->VU_LP1_L9->setPixmap(QPixmap(":/icons/led_yellow_on.png"));
    } else {
        ui->VU_LP1_L9->setPixmap(QPixmap(":/icons/led_yellow_off.png"));
    }
    if(valorPercentualEsquerdo > 80){
         ui->VU_LP1_L8->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP1_L8->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualEsquerdo > 70){
         ui->VU_LP1_L7->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP1_L7->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualEsquerdo > 60){
         ui->VU_LP1_L6->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP1_L6->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualEsquerdo > 50){
         ui->VU_LP1_L5->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP1_L5->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualEsquerdo > 40){
         ui->VU_LP1_L4->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP1_L4->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualEsquerdo > 30){
         ui->VU_LP1_L3->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP1_L3->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualEsquerdo > 20){
         ui->VU_LP1_L2->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP1_L2->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualEsquerdo > 10){
         ui->VU_LP1_L1->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP1_L1->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
...
void player::on_actionForce_to_open_default_database_triggered()
{
    checkDbOpen();
}

void player::dataReceivedLp2(const QMap<Phonon::AudioDataOutput::Channel, QVector<qint16> > map){
...
if(valorPercentualDireito>0){
    ...
    if(valorPercentualDireito > 95){
         ui->VU_LP2_R10->setPixmap(QPixmap(":/icons/led_red_on.png"));
    } else {
        ui->VU_LP2_R10->setPixmap(QPixmap(":/icons/led_red_off.png"));
    }
    if(valorPercentualDireito > 90){
         ui->VU_LP2_R9->setPixmap(QPixmap(":/icons/led_yellow_on.png"));
    } else {
        ui->VU_LP2_R9->setPixmap(QPixmap(":/icons/led_yellow_off.png"));
    }
    if(valorPercentualDireito > 80){
         ui->VU_LP2_R8->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP2_R8->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualDireito > 70){
         ui->VU_LP2_R7->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP2_R7->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualDireito > 60){
         ui->VU_LP2_R6->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP2_R6->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualDireito > 50){
         ui->VU_LP2_R5->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP2_R5->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualDireito > 40){
         ui->VU_LP2_R4->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP2_R4->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualDireito > 30){
         ui->VU_LP2_R3->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP2_R3->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualDireito > 20){
         ui->VU_LP2_R2->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP2_R2->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualDireito > 10){
         ui->VU_LP2_R1->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP2_R1->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
...
if(valorPercentualEsquerdo>0){
    ...
    if(valorPercentualEsquerdo > 95){
         ui->VU_LP2_L10->setPixmap(QPixmap(":/icons/led_red_on.png"));
    } else {
        ui->VU_LP2_L10->setPixmap(QPixmap(":/icons/led_red_off.png"));
    }
    if(valorPercentualEsquerdo > 90){
         ui->VU_LP2_L9->setPixmap(QPixmap(":/icons/led_yellow_on.png"));
    } else {
        ui->VU_LP2_L9->setPixmap(QPixmap(":/icons/led_yellow_off.png"));
    }
    if(valorPercentualEsquerdo > 80){
         ui->VU_LP2_L8->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP2_L8->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualEsquerdo > 70){
         ui->VU_LP2_L7->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP2_L7->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualEsquerdo > 60){
         ui->VU_LP2_L6->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP2_L6->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualEsquerdo > 50){
         ui->VU_LP2_L5->setPixmap(QPixmap(":/icons/led_green_on.png"));
    } else {
        ui->VU_LP2_L5->setPixmap(QPixmap(":/icons/led_green_off.png"));
    }
    if(valorPercentualEsquerdo > 40){
         ui->VU_LP2_L4->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP2_L4->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualEsquerdo > 30){
         ui->VU_LP2_L3->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP2_L3->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualEsquerdo > 20){
         ui->VU_LP2_L2->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP2_L2->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
    if(valorPercentualEsquerdo > 10){
         ui->VU_LP2_L1->setPixmap(QPixmap(":/icons/led_blue_on.png"));
    } else {
        ui->VU_LP2_L1->setPixmap(QPixmap(":/icons/led_blue_off.png"));
    }
...
void player::sourceChanged(Phonon::MediaSource){
    qDebug()<<"lauched sourceChanged phonon MAIN";



    QString newtxt = ui->txtNowPlaying_2->text();

    if(!newtxt.isEmpty()){
            ui->txtNowPlaying->setText(newtxt);
    }


    ui->txtNowPlaying_2->setText("");

}

void player::setXMarginTxtPlay2()
{

    QString stylesheet = "color: rgb(124, 124, 186);margin-left:250px";
    ui->txtNowPlaying_2->setStyleSheet(stylesheet);
}

void player::aboutToFinish(){
    qDebug()<<"lauched about to finish phonon MAIN";
if(PlayMode=="Playing_Segue"){
if(ui->playlist->count()>0){
    trocaDeMusica();
} else {
...
void player::trocaDeMusica(){

    QString itemDaPlaylist = ui->playlist->item(0)->text();
    mediaObject->enqueue(itemDaPlaylist);
    if((lastPlayedSong!=itemDaPlaylist)||(autoMode==0)){
    qDebug()<<"lastplayesong != itemdaplaylist";
    ...
    QFileInfo fileName(itemDaPlaylist);
    QString baseName = fileName.fileName();
    ui->txtNowPlaying_2->setText(baseName);
...
void player::processBuffer(const QAudioBuffer& buffer)
{
    if (audioLevels.count() != buffer.format().channelCount()) {
        ...
        for (int i = 0; i < buffer.format().channelCount(); ++i) {
            ...
            qDebug()<<"now setting level input : "<<level;
        }
    }
    ...
    int powerLevel = levels.at(i)*190;

        //qDebug()<<"now setting level input as: "<<p;
        //


        if(powerLevel > 95){
             ui->VU_MAIN_R10_IN->setPixmap(QPixmap(":/icons/led_red_on.png"));
             ui->VU_MAIN_L10_IN->setPixmap(QPixmap(":/icons/led_red_on.png"));
        } else {
            ui->VU_MAIN_R10_IN->setPixmap(QPixmap(":/icons/led_red_off.png"));
            ui->VU_MAIN_L10_IN->setPixmap(QPixmap(":/icons/led_red_off.png"));
        }
        if(powerLevel > 90){
             ui->VU_MAIN_R9_IN->setPixmap(QPixmap(":/icons/led_yellow_on.png"));
             ui->VU_MAIN_L9_IN->setPixmap(QPixmap(":/icons/led_yellow_on.png"));
        } else {
            ui->VU_MAIN_R9_IN->setPixmap(QPixmap(":/icons/led_yellow_off.png"));
            ui->VU_MAIN_L9_IN->setPixmap(QPixmap(":/icons/led_yellow_off.png"));
        }
        if(powerLevel > 80){
             ui->VU_MAIN_R8_IN->setPixmap(QPixmap(":/icons/led_green_on.png"));
             ui->VU_MAIN_L8_IN->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_R8_IN->setPixmap(QPixmap(":/icons/led_green_off.png"));
            ui->VU_MAIN_L8_IN->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(powerLevel > 70){
             ui->VU_MAIN_R7_IN->setPixmap(QPixmap(":/icons/led_green_on.png"));
             ui->VU_MAIN_L7_IN->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_R7_IN->setPixmap(QPixmap(":/icons/led_green_off.png"));
            ui->VU_MAIN_L7_IN->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(powerLevel > 60){
             ui->VU_MAIN_R6_IN->setPixmap(QPixmap(":/icons/led_green_on.png"));
             ui->VU_MAIN_L6_IN->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_R6_IN->setPixmap(QPixmap(":/icons/led_green_off.png"));
            ui->VU_MAIN_L6_IN->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(powerLevel > 50){
             ui->VU_MAIN_R5_IN->setPixmap(QPixmap(":/icons/led_green_on.png"));
             ui->VU_MAIN_L5_IN->setPixmap(QPixmap(":/icons/led_green_on.png"));
        } else {
            ui->VU_MAIN_R5_IN->setPixmap(QPixmap(":/icons/led_green_off.png"));
            ui->VU_MAIN_L5_IN->setPixmap(QPixmap(":/icons/led_green_off.png"));
        }
        if(powerLevel > 40){
             ui->VU_MAIN_R4_IN->setPixmap(QPixmap(":/icons/led_blue_on.png"));
             ui->VU_MAIN_L4_IN->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_R4_IN->setPixmap(QPixmap(":/icons/led_blue_off.png"));
            ui->VU_MAIN_L4_IN->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }
        if(powerLevel > 30){
             ui->VU_MAIN_R3_IN->setPixmap(QPixmap(":/icons/led_blue_on.png"));
             ui->VU_MAIN_L3_IN->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_R3_IN->setPixmap(QPixmap(":/icons/led_blue_off.png"));
            ui->VU_MAIN_L3_IN->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }
        if(powerLevel > 20){
             ui->VU_MAIN_R2_IN->setPixmap(QPixmap(":/icons/led_blue_on.png"));
             ui->VU_MAIN_L2_IN->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_R2_IN->setPixmap(QPixmap(":/icons/led_blue_off.png"));
            ui->VU_MAIN_L2_IN->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }
        if(powerLevel > 10){
             ui->VU_MAIN_R1_IN->setPixmap(QPixmap(":/icons/led_blue_on.png"));
             ui->VU_MAIN_L1_IN->setPixmap(QPixmap(":/icons/led_blue_on.png"));
        } else {
            ui->VU_MAIN_R1_IN->setPixmap(QPixmap(":/icons/led_blue_off.png"));
            ui->VU_MAIN_L1_IN->setPixmap(QPixmap(":/icons/led_blue_off.png"));
        }
